...
import com.heqi.image.LoadImageCallback;
...
public class AsyncImageView extends ImageView {
  ...
  /**
   * Sets {@link ImageManagerHolder} before any other method is called.
   *
  ...
  protected void onAttachedToWindow() {
    // TODO @xubin onLayout is not called when re-attach ofter detach, check this logic
    resumeLoading();
  ...
  private void setStatus(AsyncTask.Status status) {
    ...
    AsyncTask.Status originStatus = this.status;
    this.status = status;
   ...
   * {@link #setImageBitmap(Bitmap)} is that this clears previous state before
   * setting image.
   *
   * @param bitmap bitmap
   */
  ...
  protected void loadNetImageAsync(final boolean isInLayout) {
    ...
    if (requiredHeight != 0 && requiredWidth != 0) {
      ...
      // save memory in api 2.3, by yangkai@wandoujia.com
  ...
  public interface OnStatusChangedListener {
     ...
     */
    void onStatusChanged(AsyncTask.Status originStatus, AsyncTask.Status targetStatus);
  }
